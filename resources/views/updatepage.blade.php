...
@section('main-section')
    ...
    <div class="form-box p-4">
        ...
        <form action="{{ url('/') }}/updatedata/{{ $userData->id }}" method="post" class="form">
            @csrf
            <div>
                <h1 class="text-center">UPDATE SINGLE USER</h1>
            </div>

            <div class="form-group">
                <label for="">username</label>
                <input type="text" value="{{ $userData->name }}" class="form-control" name="name"
                    placeholder="please enter your name">
            </div>
            <div class="form-group">
                <label for="">email</label>
                <input type="text" value="{{ $userData->email }}" class="form-control" name="email"
                    placeholder="please enter your email">
            </div>
            <div class="form-group">
                <label for="">phone</label>
                <input type="phone" value="{{ $userData->phone }}" class="form-control" name="phone"
                    placeholder="please enter your phone">
            </div>
            <label for="">address</label>
            <div class="form-group">
                <textarea style="resize: none;" name="address" id="address" cols="70" rows="5"
                    placeholder="please enter your address">{{ $userData->address }}</textarea>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <select class="form-control" name="gender" id="state">
                    ...
                    @else
                        $variable='selected';
                    @endif
                    <option value="1" {{ $variable }}>Male</option>
                    <option value="2" {{ $variable }}>Female</option>
                    <option value="0" {{ $variable }}>Custom</option>
                </select>
            </div>
            <div class="form-group">
